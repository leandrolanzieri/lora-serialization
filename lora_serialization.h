...
/**
 * @defgroup    lora-serialization Lora serialization
 * @brief       Lora serialization library
 * @{
 * @file
...
/**
 * @brief Lora Serialization bitmap
 */
typedef struct {
	unsigned char B0:1;
	unsigned char B1:1;
	...
	unsigned char B6:1;
	unsigned char B7:1;
} lora_serialization_bitmap_t;

/**
 * @brief Lora Serialization descriptor
 */
typedef struct {
    uint8_t buffer[LORA_SERIALIZATION_MAX_BUFFER_SIZE];
    uint8_t cursor;
 ...
 * 
 * @param serialization[in]      lora serialization descriptor
 * @param bitmap[in]             bitmap value
 */
